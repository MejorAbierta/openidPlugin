...
$(function() {ldelim}
	 ...
	 $('#addNewProvider').click(function() {ldelim}
		 $('#newProviderKey').val('');
		 $('#newProviderName').val('');
	 ...
	 $('#cancelAddProvider').click(function() {ldelim}
		 $('#addProviderModal').pkpModal('hide');
	 {rdelim});

	 // Handle confirm button
	 $('#confirmAddProvider').click(function() {ldelim}
		 const providerKey = $('#newProviderKey').val();
		 const providerName = $('#newProviderName').val();
		 const configUrl = $('#newProviderConfigUrl').val();
		 
		 if (!providerKey || !providerName || !configUrl) {ldelim}
			 alert("plugins.generic.openid.settings.provider.requiredFields");
			 return;
		 {rdelim}

		 // Add the new provider via AJAX
		 $.ajax({ldelim}
			 url: '{url router=$smarty.const.ROUTE_COMPONENT op="manage" category="generic" plugin=$pluginName verb="addProvider"}',
			 type: 'POST',
			 data: {ldelim}
				 csrfToken: '{$csrfToken}',
			 ...
			 success: function(response) {ldelim}
				 if (response.status) {ldelim}
					 location.reload(); // Refresh to show the new provider
				 {rdelim} else {ldelim}
...
<style>
 ...
 /* Styles for Add Provider button */
 .add-provider-container {
	 margin-bottom: 20px;
 }
 
 /* Styles for custom providers */
 .custom-provider {
...
<div class="add-provider-container">
 <button type="button" id="addNewProvider" class="pkp_button">
...
<form
 ...
 {fbvFormArea title="plugins.generic.openid.settings.openid.head" id="open-id-provider"}
	 ...
	 {* Custom Providers *}
	 {foreach from=$customProviders item=settings key=name}
		 <div class="showContent custom-provider">
			 {fbvFormSection list=true style="padding: 0;" class="provider_list"}
				 {fbvElement type="checkbox" id="provider[{$name}][active]" checked=$provider[{$name}]['active']  value=1 label=$settings.displayName class="strong"}
				 <div class="hiddenContent">
					 {assign var='providerSuffix' value="?provider="|cat:$name}
					 <p>
						 {translate key="plugins.generic.openid.settings.custom.provider.desc"}
						 &nbsp;
						 <strong>
							 {$redirectUrl|escape}{$providerSuffix}
						 </strong>
					 ...
					 <div class="pkp_form_actions">
						 <button type="button" class="pkp_button pkp_button_offset deleteProvider" data-provider="{$name}">
...
<div id="deleteProviderModal" class="pkp_modal" style="display: none;">
 <div class="pkp_modal_panel">
	 <div class="header">
		 <h4>{translate key="plugins.generic.openid.settings.deleteProvider.title"}</h4>
	 </div>
	 <div class="content">
		 <p>{translate key="plugins.generic.openid.settings.deleteProvider.confirm"}</p>
	 </div>
	 <div class="footer">
		 <button class="pkp_button" id="cancelDeleteProvider">{translate key="common.cancel"}</button>
		 <button class="pkp_button pkp_button_warning" id="confirmDeleteProvider">{translate key="common.delete"}</button>
	 </div>
 </div>
</div>
...
/*
 ...
 $(function() {
	 ...
	 $('.deleteProvider').click(function() {
		 providerToDelete = $(this).data('provider');
		 $('#deleteProviderModal').pkpModal('show');
	 });
	 
	 $('#cancelDeleteProvider').click(function() {
		 providerToDelete = null;
		 $('#deleteProviderModal').pkpModal('hide');
	 });
	 
	 $('#confirmDeleteProvider').click(function() {
		 if (!providerToDelete) return;
		 
		 $.ajax({
			 url: '{url router=$smarty.const.ROUTE_COMPONENT op="manage" category="generic" plugin=$pluginName verb="deleteProvider"}',
			 type: 'POST',
			 data: {
				 csrfToken: '{$csrfToken}',
				 providerKey: providerToDelete
			 },
			 success: function(response) {
				 if (response.status) {
					 location.reload(); // Refresh to remove the provider
				 } else {
					 alert(response.message);
				 }
			 },
			 error: function() {
				 alert({translate key="common.error"|json_encode});
			 }
		 });
	 });
 });*/
